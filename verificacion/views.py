...
from metrologia.models import EspecificacionMetrologia, CriteriosMetrologicos
...
class DetallesVerView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        unidad_error = CriteriosMetrologicos.objects.filter(id_controlMetro=pk, id_criterio="Error").first().id_unidad_criterio
...
class AgregarEspVerView(generic.CreateView):
    ...
    def get_initial(self):
        ...
        if id_equipo is not None:
            ...
            eq_sin_esp_ids = list(EspecificacionMetrologia.objects.exclude(
...
class ModificarEspView(generic.UpdateView):
    ...
    def get_success_url(self):
        id_equipo = self.kwargs.get('pk', None)
        messages.success(self.request, f"La modificación del periodo de verificacion del equipo {id_equipo} fue realizada exitosamente")
    ...
    def form_valid(self, form):
        try:
            with transaction.atomic():
                ...
                espMetro = EspecificacionMetrologia.objects.filter(id_equipo = self.kwargs['pk']).first()

                ver = form.save(commit=False)

                if ver.doc:
                    # Genera un nuevo nombre único para el archivo
                    extension = ver.doc.name.split('.')[-1]  # Obtiene la extensión del archivo
                    ultima  = Verificacion.objects.last()
                    if ultima:
                        n_informe = ultima.num_informe + 1
                    else:
                        n_informe = 1
                        
                    new_name = f'Informe_verificacion_{n_informe}.{extension}'
                    
                    # Asigna el nuevo nombre al archivo de imagen
                    ver.doc.name = new_name

                error = CriteriosMetrologicos.objects.filter(id_controlMetro = espMetro, id_criterio = "Error").first()
                valorEsp = error.valor_esperado
                rangoEsp = error.rango
                dictamen = ""
                print(f"Error esperado: {rangoEsp} {valorEsp}")

                if str(rangoEsp) == "=":
                    if ver.errorVer1 == valorEsp and ver.errorVer2 == valorEsp and ver.errorVer3 == valorEsp and ver.errorVer4 == valorEsp and ver.errorVer5 == valorEsp:
                        dictamen = "Conforme"
                    else:
                        dictamen = "No conforme"
                elif str(rangoEsp) == "Max":
                    if ver.errorVer1 <= valorEsp and ver.errorVer2 <= valorEsp and ver.errorVer3 <= valorEsp and ver.errorVer4 <= valorEsp and ver.errorVer5 <= valorEsp:
                        dictamen = "Conforme"
                    else:
                        dictamen = "No conforme"
                elif str(rangoEsp) == "Min":
                    if ver.errorVer1 >= valorEsp and ver.errorVer2 >= valorEsp and ver.errorVer3 >= valorEsp and ver.errorVer4 >= valorEsp and ver.errorVer5 >= valorEsp:
                        dictamen = "Conforme"
                    else:
                        dictamen = "No conforme"
                ...
                especificacion.save()
                messages.success(self.request, "Verificación registrada exitosamente")
                return super(RegistrarVerificacionView, self).form_valid(form)
        except Exception as e:
            messages.error(self.request, f"Error al guardar la verificación: {e}")
            return super(RegistrarVerificacionView, self).form_invalid(form)

    def get_success_url(self):
        informe = self.object.num_informe
        return reverse('verificacion:detalles', kwargs={'pk': informe})
...
class HistorialVerificacionView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for equipo in registros:
            indicadores[equipo.id_verificacion] = {
                'dirEsp': equipo.id_verificacion.id_equipo.id_direccion_esp,
                'sede': equipo.id_verificacion.id_equipo.id_sede,
                'clasificacion': equipo.id_verificacion.id_equipo.id_clasificacion,
                'emailResp': equipo.id_responsable.email,
                'nombreResp': equipo.id_responsable,
            }
            print(equipo.id_verificacion,": ",indicadores[equipo.id_verificacion])
